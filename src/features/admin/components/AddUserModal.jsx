    ...
    <div className="relative" ref={ref}>
      ...
      <div
        ...
        className={`min-h-[44px] w-full rounded-xl bg-[#f7f8fa] border px-2.5 py-2 flex flex-wrap items-center gap-1.5 cursor-pointer transition-all ${
          open ? 'bg-white border-[#3D7FFF] ring-4 ring-[#3D7FFF]/20' : 'border-[#0A0A0A]/10'
        }`}
      >
        {selected.length === 0 && <span className="text-sm text-gray-400 px-1">Select one or more roles…</span>}
        {selected.map((o) => (
          <span key={o.id} className="inline-flex items-center gap-1 rounded-lg bg-[#0A0A0A] text-white text-[11px] font-semibold pl-2.5 pr-1 py-1">
            {o.name}
            <button
              type="button"
        ...
        <svg className={`w-4 h-4 text-[#0A0A0A]/40 ml-auto shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M6 9l6 6 6-6" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </div>
      ...
      {open && dropdownStyle && createPortal(
        <div
          ...
          className="rounded-xl bg-white border border-[#0A0A0A]/10 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150"
        >
          <div className="p-2 border-b border-[#0A0A0A]/[0.06]">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg bg-[#f7f8fa] pl-8 pr-3 py-2 text-sm text-[#0A0A0A] placeholder-gray-400 outline-none focus:bg-white focus:ring-2 focus:ring-[#3D7FFF]/40"
              />
            </div>
          </div>
              ...
              filtered.map((o) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => toggle(o.id)}
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-[#0A0A0A] hover:bg-[#3D7FFF]/[0.12] transition-colors text-left"
                  >
                    <span className={`w-4 h-4 rounded-[5px] flex items-center justify-center shrink-0 border ${isSel ? 'bg-[#0A0A0A] border-[#0A0A0A]' : 'border-[#0A0A0A]/25'}`}>
                      {isSel && <svg className="w-3 h-3 text-[#3D7FFF]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                    </span>
...
export const AddUserModal = ({ isOpen, onClose, onSubmit, editingUser, fetchRoles, fetchEntities }) => {
  ...
  return createPortal(
    ...
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#0A0A0A]/40 backdrop-blur-sm animate-in fade-in duration-200"
      onClick={onClose}
      role="presentation"
    >
      <div
        className="relative w-full max-w-[600px] max-h-[92vh] overflow-y-auto bg-white rounded-3xl border border-[#0A0A0A]/[0.06] shadow-[0_20px_60px_rgba(27,30,38,0.25)] p-6 sm:p-8 animate-in zoom-in-95 fade-in duration-200"
