...
const inputClass =
  'w-full rounded-xl bg-[#f7f8fa] border border-[#0A0A0A]/10 px-3.5 py-2.5 text-[13px] text-[#0A0A0A] placeholder-gray-400 focus:bg-white focus:border-[#3D7FFF] focus:ring-2 focus:ring-[#3D7FFF]/25 focus:outline-none transition-all';
...
const ModalShell = ({ title, subtitle, icon, onClose, children, maxW = '480px' }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#0A0A0A]/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} role="presentation">
      <div
        className="relative w-full bg-white rounded-3xl border border-[#0A0A0A]/[0.06] shadow-[0_20px_60px_rgba(18,14,26,0.25)] p-6 sm:p-7 animate-in zoom-in-95 fade-in duration-200"
        style={{ maxWidth: maxW }}
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="pt-1 flex justify-end gap-2.5">
          <button type="button" onClick={onClose} className="px-5 py-2 rounded-xl text-[13px] font-semibold text-[#0A0A0A]/70 border border-[#0A0A0A]/10 hover:bg-[#f3f4f6] transition-colors">Cancel</button>
          <button type="submit" disabled={saving} className="px-6 py-2 rounded-xl text-[13px] font-bold bg-[#0A0A0A] text-white hover:bg-black transition-colors active:scale-[0.98] shadow-sm disabled:opacity-60">
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        >
          <span className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${required ? 'bg-[#3D7FFF] border-[#3D7FFF] text-white' : 'bg-white border-[#0A0A0A]/15 text-transparent'}`}>
          ...
          <span className="text-[13px] font-medium text-[#0A0A0A]/80">Required to complete the module</span>
        </button>
...
export const ConfirmDeleteModal = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#0A0A0A]/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} role="presentation">
      <div
        className="relative w-full max-w-sm bg-white rounded-3xl border border-[#0A0A0A]/[0.06] shadow-[0_20px_60px_rgba(18,14,26,0.25)] p-5 animate-in zoom-in-95 fade-in duration-200"
        onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true"
      >
        ...
        <div className="flex flex-col items-center text-center pt-1">
          ...
          <p className="text-[13px] text-[#0A0A0A]/60 leading-snug mt-1 max-w-xs">
            {message}
            {itemName && (
              <><strong className="text-[#0A0A0A]/80"> {itemName}</strong>.</>
            )}
          </p>
        </div>

        <div className="mt-4 pt-3.5 border-t border-[#0A0A0A]/8 flex justify-end gap-2.5">
          <button type="button" onClick={onClose} disabled={busy} className="px-4 py-2 rounded-xl text-[12px] font-semibold text-[#0A0A0A]/60 border border-[#0A0A0A]/12 hover:bg-[#f3f4f6] hover:text-[#0A0A0A]/80 transition-colors disabled:opacity-50">Cancel</button>
