...
// ── Avatar helpers ────────────────────────────────────────────────────────────
const AVATAR_STYLES = [
  'bg-[#3D7FFF]/20 text-white', 'bg-rose-100 text-rose-700',
...
const filterFieldClass =
  'w-full text-[13px] px-3 py-2 rounded-lg border border-[#0A0A0A]/10 bg-[#f7f8fa] text-[#0A0A0A] focus:bg-white focus:ring-4 focus:ring-[#3D7FFF]/20 focus:border-[#3D7FFF] focus:outline-none transition-all';
const filterSelectClass = `${filterFieldClass} appearance-none pr-8 cursor-pointer`;
const filterLabelClass = 'text-[10px] font-semibold text-[#0A0A0A]/45 uppercase tracking-[0.12em]';

const SelectChevron = () => (
  <svg className="w-3.5 h-3.5 text-[#0A0A0A]/40 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
...
export const UserManagementPage = () => {
  ...
  const columns = [
      ...
      render: (u) => {
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-[#0A0A0A] whitespace-nowrap">{u.name || '—'}</span>
            {isSelf(u) && (
              <span className="px-1.5 py-0.5 rounded-md bg-[#3D7FFF] text-white text-[9px] font-bold uppercase tracking-wide shrink-0">
                You
              </span>
            )}
          </div>
    ...
    },
    {
      ...
      render: (u) => <span className="text-[13px] text-[#0A0A0A]/60">{u.username || '—'}</span>,
    },
    {
      key: 'email', header: 'Email', sortable: true, sortValue: (u) => u.email || '',
      render: (u) => <span className="text-[#0A0A0A]/45">{u.email || '—'}</span>,
    },
    {
          ...
          <div className="flex flex-wrap gap-1.5">
            {list.map((r) => {
              ...
              return (
                <span
                  key={r?.id || name}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-[#f7f8fa] text-[#0A0A0A]/80 border border-[#0A0A0A]/10"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-[#3D7FFF] ring-1 ring-[#0A0A0A]/20 shrink-0" />
                  {name}
                </span>
              );
            })}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <div>
          <h1 className="text-[19px] font-medium text-[#0A0A0A] tracking-tight">User Management</h1>
      ...
      <div className="bg-white rounded-xl border border-[#0A0A0A]/[0.06] shadow-sm px-3 py-2.5">
        <div className="flex flex-wrap items-end gap-2.5">
          ...
          <button
            onClick={fetchUsers}
            className="shrink-0 ml-auto h-[34px] px-5 rounded-full bg-[#3D7FFF] text-white text-[13px] font-semibold hover:bg-[#63C7FF] shadow-sm transition-colors active:scale-[0.98]"
          >
            Apply
          </button>
          <button
