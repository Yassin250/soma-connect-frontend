...
const StatusToggleModal = ({ category, onClose, onConfirm }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1b1e26]/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} role="presentation">
      <div
        className="relative w-full max-w-[420px] bg-white rounded-3xl border border-[#1b1e26]/[0.06] shadow-[0_20px_60px_rgba(27,30,38,0.25)] animate-in zoom-in-95 fade-in duration-200 overflow-hidden"
        onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true"
      >
        <div className="flex items-start justify-between px-6 pt-6 pb-4">
          <div>
            <h3 className="text-lg font-bold text-[#1b1e26] tracking-tight">{willActivate ? 'Activate' : 'Deactivate'}</h3>
            <span className="block w-8 h-[3px] rounded-full bg-[#d0f24a] mt-1.5" />
          </div>
        ...
        <div className="px-6 py-5 border-t border-b border-[#1b1e26]/[0.06] bg-[#f9fbf3]">
          <p className="text-[14px] text-[#1b1e26]/70 text-center leading-relaxed">
            Are you sure you want to {willActivate ? 'activate' : 'deactivate'}{' '}
            <span className="font-bold text-[#1b1e26]">"{category.name}"</span>?
          </p>

          <div className="flex items-center justify-center gap-3 mt-5">
            <span className={`text-[13px] font-semibold ${!willActivate ? 'text-[#1b1e26]' : 'text-[#1b1e26]/35'}`}>Inactive</span>
            <button
              type="button"
              onClick={confirm}
              disabled={saving}
              aria-label={willActivate ? 'Activate' : 'Deactivate'}
              className={`relative w-12 h-7 rounded-full transition-colors disabled:opacity-60 ${willActivate ? 'bg-[#1b1e26]/15' : 'bg-[#d0f24a]'}`}
            >
              <span className={`absolute top-1 w-5 h-5 rounded-full bg-white shadow-sm transition-all ${willActivate ? 'left-1' : 'left-6'}`} />
            </button>
            <span className={`text-[13px] font-semibold ${willActivate ? 'text-[#1b1e26]' : 'text-[#1b1e26]/35'}`}>Active</span>
          </div>
        </div>

        <div className="flex justify-center gap-2.5 px-6 py-4">
          <button type="button" onClick={onClose} className="px-5 py-2 rounded-xl text-[13px] font-semibold text-[#1b1e26]/70 border border-[#1b1e26]/10 hover:bg-[#f3f4f6] transition-colors">
            Cancel
          </button>
          <button
            type="button"
            onClick={confirm}
            disabled={saving}
            className="px-6 py-2 rounded-xl text-[13px] font-bold bg-[#1b1e26] text-white hover:bg-black transition-colors active:scale-[0.98] shadow-sm disabled:opacity-60"
          >
...
const CategoryModal = ({ open, editing, onClose, onSubmit }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1b1e26]/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} role="presentation">
      <div
        className="relative w-full max-w-[480px] bg-white rounded-3xl border border-[#1b1e26]/[0.06] shadow-[0_20px_60px_rgba(27,30,38,0.25)] p-6 sm:p-8 animate-in zoom-in-95 fade-in duration-200"
        onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true"
      >
        <button type="button" onClick={onClose} aria-label="Close" className="absolute top-4 right-4 w-9 h-9 rounded-xl flex items-center justify-center text-[#1b1e26]/40 hover:text-[#1b1e26] hover:bg-[#f3f4f6] transition-colors">
        ...
        <div className="mb-6 pr-8">
          ...
          <h3 className="text-xl font-semibold text-[#1b1e26] tracking-tight">{editing ? 'Update category' : 'New category'}</h3>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="pt-2 flex justify-end gap-2.5">
            <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-[#1b1e26]/70 border border-[#1b1e26]/10 hover:bg-[#f3f4f6] transition-colors">Cancel</button>
            <button type="submit" disabled={saving} className="px-6 py-2.5 rounded-xl text-sm font-bold bg-[#d0f24a] text-[#1b1e26] hover:bg-[#c4e83a] transition-colors active:scale-[0.98] shadow-sm disabled:opacity-60">
  ...
  const columns = [
    {
      ...
      render: (c) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-medium text-[#1b1e26]">{c.name}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (c) => (
        c.courseCount > 0 ? (
          ...
          <span className="inline-flex items-center justify-center min-w-[1.75rem] h-7 px-2 rounded-full bg-[#1b1e26]/[0.06] text-[#1b1e26]/40 text-[11px] font-bold">0</span>
        )
      ),
    },
    {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-[19px] font-medium tracking-tight mt-1 text-[#1b1e26]">Course Categories</h1>
      ...
      {/* Filter Toolbar */}
      <div className="bg-white rounded-xl border border-[#1b1e26]/[0.06] shadow-sm px-3 py-2.5 flex flex-wrap items-end gap-3">
        {/* Search */}
        <div className="w-[240px] flex flex-col gap-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full text-[13px] px-3 py-2 pl-9 rounded-lg border border-[#1b1e26]/10 bg-[#f7f8fa] text-[#1b1e26] focus:bg-white focus:ring-2 focus:ring-[#d0f24a]/25 focus:border-[#d0f24a] focus:outline-none transition-all"
            />
          </div>
        </div>
        ...
        <div className="w-[130px] flex flex-col gap-1">
          ...
          <select
            className="w-full text-[13px] px-3 py-2 rounded-lg border border-[#1b1e26]/10 bg-[#f7f8fa] text-[#1b1e26] cursor-pointer focus:bg-white focus:ring-2 focus:ring-[#d0f24a]/25 focus:border-[#d0f24a] focus:outline-none transition-all"
            value={statusFilter}
        ...
        <div className="w-[180px] flex flex-col gap-1">
          ...
          <div className="relative">
            <svg className="w-4 h-4 text-[#1b1e26]/35 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
        ...
        <button
          ...
          className="text-[#1b1e26]/60 text-[13px] font-semibold px-4 py-2 rounded-lg border border-[#1b1e26]/10 hover:bg-[#f7f8fa] transition-colors"
        >
          Reset
        </button>
      </div>
