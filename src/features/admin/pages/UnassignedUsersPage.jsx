  ...
  const columns = [
    {
      ...
      render: (u) => (
        <input
          ...
          className="w-4 h-4 rounded border-gray-300 text-[#d0f24a] focus:ring-[#d0f24a] cursor-pointer accent-[#1b1e26]"
        />
      ),
      headerRender: () => (
        <input
    ...
    },
    {
      ...
      render: (u) => {
        if (!u.createdAt) return <span className="text-[13px] text-[#1b1e26]/30">—</span>;
        const d = new Date(u.createdAt);
        return <span className="text-[13px] text-[#1b1e26]/70">{d.toLocaleDateString()}</span>;
      },
    },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-[19px] font-medium tracking-tight mt-1 text-[#1b1e26]">Unassigned Users</h1>
        ...
        <button
          ...
          className="shrink-0 px-4 py-2 rounded-xl bg-[#1b1e26] text-white text-sm font-semibold hover:bg-black transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-[#1b1e26]">Assign Users to Entity</h2>
              <button onClick={() => !assigning && setShowAssignModal(false)} className="w-8 h-8 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-[#1b1e26] flex items-center justify-center transition-colors">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-semibold text-[#1b1e26]/60 uppercase tracking-[0.1em]">Target Institution</label>
              <select
              ...
              >
                ...
                {entities.map((e) => (
                  <option key={e.id} value={e.id} className="text-[#1b1e26]">{e.name} ({e.code})</option>
                ))}
              </select>
            </div>
