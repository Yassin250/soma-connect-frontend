...
export const CoursesPage = () => {
  ...
  const columns = [
      ...
      render: (c) => (
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-medium text-[#1b1e26]/90 truncate max-w-[280px]">{c.title}</p>
    ...
    },
    {
      ...
      render: (c) => <span className="text-[13px] text-[#1b1e26]/70">{c.entityName || '—'}</span>,
    },
    {
    ...
    },
    {
      ...
      render: (c) => <span className="text-[13px] text-[#1b1e26]/60">{humanize(c.deliveryMode)}</span>,
    },
    {
      ...
      render: (c) => (
        <select
          ...
          disabled
          className="px-2.5 py-1 text-[12px] font-medium rounded-lg border border-[#1b1e26]/10 bg-[#f7f8fa] text-[#1b1e26]/80 cursor-default"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <h1 className="text-[19px] font-medium tracking-tight mt-1 text-[#1b1e26]">Courses</h1>
        <p className="text-[12px] text-gray-500 mt-0.5">Every course published by institutions across the platform.</p>
        {categoryFilter && (
          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#d0f24a]/20 text-[#1b1e26] pl-3.5 pr-2 py-1.5 text-[13px] font-semibold">
            <span className="text-[#1b1e26]/50 font-medium">Category:</span>
            {categoryFilter}
            <button
              onClick={clearCategory}
              aria-label="Clear category filter"
              className="w-5 h-5 rounded-full flex items-center justify-center text-[#1b1e26]/50 hover:text-[#1b1e26] hover:bg-[#1b1e26]/10 transition-colors"
            >
        ...
        <button
          ...
          className="bg-[#1b1e26] text-white text-[13px] font-semibold px-4 py-2 rounded-xl hover:bg-black transition-colors inline-flex items-center gap-2 shadow-sm active:scale-[0.98] shrink-0"
        >
      ...
      <div className="bg-white rounded-xl border border-[#1b1e26]/[0.06] shadow-sm px-3 py-2.5 flex flex-wrap items-end gap-x-2.5 gap-y-2">
        ...
        <div className="w-[200px] flex flex-col gap-1">
          <label className="text-[10px] font-semibold text-[#1b1e26]/45 uppercase tracking-[0.12em]">Institution</label>
          <select
        ...
        <div className="w-[150px] flex flex-col gap-1">
          ...
          <div className="relative">
            <svg className="w-3.5 h-3.5 text-[#1b1e26]/35 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
