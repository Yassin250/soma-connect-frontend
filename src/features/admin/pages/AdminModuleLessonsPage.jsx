  ...
  const columns = [
    {
      ...
      render: (l) => (
        <span className="w-7 h-7 rounded-lg bg-[#1b1e26]/[0.06] text-[#1b1e26]/50 text-[11px] font-bold flex items-center justify-center">
          {l._order}
        </span>
      ),
    },
      ...
      render: (l) => {
        ...
        return (
          <div className="flex items-center gap-3 min-w-0">
            ...
            <p className="font-medium text-[#1b1e26] truncate max-w-[340px]">{l.title || 'Untitled lesson'}</p>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      render: (l) => (
        <span className="text-[13px] text-[#1b1e26]/60">{l.durationMinutes ? `${l.durationMinutes} min` : '—'}</span>
      ),
    },
    {
      ...
      render: (l) => (
        l.contentUrl
          ? (/^https?:\/\//i.test(l.contentUrl)
              ? <a href={l.contentUrl} target="_blank" rel="noreferrer" className="text-[13px] text-[#5b6b12] hover:text-[#1b1e26] hover:underline truncate max-w-[220px] inline-block align-middle">{l.contentUrl}</a>
              : <span className="text-[13px] text-[#1b1e26]/50 truncate max-w-[220px] inline-block align-middle">{l.contentUrl}</span>)
          : <span className="text-[13px] text-[#1b1e26]/30">—</span>
      ),
    },
    {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          disabled={!module}
          className="bg-[#1b1e26] text-white text-[13px] font-semibold px-4 py-2 rounded-lg hover:bg-black transition-colors inline-flex items-center gap-2 shadow-sm active:scale-[0.98] shrink-0 disabled:opacity-50"
        >
