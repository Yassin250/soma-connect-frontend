...
export const AdminLayout = () => {
  ...
  return (
    <div className="h-screen w-full flex bg-[#f3f4f6] text-[#0A0A0A] antialiased overflow-hidden">
      ...
      {/* ── Sidebar ── */}
      <aside
        className={`bg-[#0A0A0A] flex flex-col fixed inset-y-0 left-0 z-50 lg:static lg:z-auto transition-all duration-300 ${
      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 shrink-0">
          ...
          {!collapsed && (
            <p className="mt-2 ml-[48px] text-[9px] font-black text-[#3D7FFF] uppercase tracking-[0.25em]">Super Admin Portal</p>
          )}
        </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition-all duration-200 ${
                ...
                isActive
                      ? 'bg-[#3D7FFF] text-white shadow-sm'
            ...
            return (
              <div key={section.id}>
                ...
                >
                  ...
                  {!railMode && (
                    <svg
                      className={`w-3.5 h-3.5 shrink-0 transition-transform duration-300 ${expanded ? 'rotate-180 text-[#3D7FFF]' : 'text-white/30'}`}
                      ...
                      return (
                        <NavLink
                          ...
                          className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-[12px] transition-all duration-200 ${
                            active
                               ? 'bg-[#3D7FFF] text-white font-semibold shadow-sm'
        ...
        {!collapsed && (
          <div className="p-3 shrink-0">
            <div className="rounded-2xl bg-[#3D7FFF] p-4 relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-[72px] shrink-0 bg-white border-b border-gray-100 flex items-center gap-4 px-4 sm:px-6">
          <button onClick={() => setMobileOpen(true)} className="lg:hidden text-gray-500 hover:text-[#0A0A0A] p-1" aria-label="Open menu">
          ...
          <button
            ...
            className="hidden lg:flex text-gray-400 hover:text-[#0A0A0A] hover:bg-gray-100 w-9 h-9 rounded-lg items-center justify-center transition-colors"
          ...
          <div className="hidden sm:block min-w-0">
            ...
            <h1
              key={pageTitle}
              className="text-base font-semibold text-[#0A0A0A] leading-tight truncate animate-in fade-in slide-in-from-bottom-1 duration-300"
            >
              {pageTitle}
            </h1>
          </div>
          ...
          <div className="flex-1 max-w-md ml-auto hidden md:block">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search…"
                className="w-full rounded-xl bg-gray-100 py-2.5 pl-10 pr-4 text-sm text-gray-800 placeholder-gray-400 outline-none border border-transparent focus:bg-white focus:border-[#0A0A0A]/15 focus:ring-4 focus:ring-[#0A0A0A]/5 transition-all"
              />
            </div>
          </div>
          ...
          <div className="relative" ref={profileRef}>
            <button onClick={() => setProfileOpen((o) => !o)} className="group flex items-center gap-2.5 pl-1 pr-2 py-1 rounded-xl hover:bg-gray-100 transition-colors">
              <span className="w-9 h-9 rounded-full bg-gradient-to-br from-[#3D7FFF] to-[#63C7FF] text-white text-xs font-bold flex items-center justify-center ring-0 ring-[#3D7FFF]/0 group-hover:ring-4 group-hover:ring-[#3D7FFF]/40 transition-all duration-200">
                {initialsOf(user?.name || user?.username)}
              </span>
              <div className="hidden sm:block text-left min-w-0">
                <p className="text-sm font-semibold text-[#0A0A0A] leading-tight max-w-[200px] truncate">{user?.name || user?.username || 'Admin'}</p>
            ...
            {profileOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="px-4 py-3 border-b border-gray-50 flex items-center gap-3">
                  <span className="w-10 h-10 rounded-full bg-gradient-to-br from-[#3D7FFF] to-[#63C7FF] text-white text-xs font-bold flex items-center justify-center shrink-0">
                    {initialsOf(user?.name || user?.username)}
                  </span>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-[#0A0A0A] truncate">{user?.name || user?.username}</p>
                    <p className="text-xs text-gray-400 truncate">{user?.email || '—'}</p>
                  </div>
                </div>
                <button
                  onClick={() => { setProfileOpen(false); navigate('/admin/account'); }}
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-600 hover:text-[#0A0A0A] hover:bg-gray-50 transition-colors"
                >
