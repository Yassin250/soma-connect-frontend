...
const inputClass =
  'w-full rounded-xl bg-white border border-[#0B0E17]/10 px-3.5 py-2.5 text-sm text-[#1b1e26] placeholder-gray-400 focus:border-accent focus:ring-4 focus:ring-accent/20 focus:outline-none transition-all';

const DetailCard = ({ label, value, children }) => (
  <div className="rounded-xl bg-[#f7f8fa] border border-[#0B0E17]/[0.05] px-4 py-3.5">
...
export const LecturerAccountPage = () => {
  const { user } = useAuth();
  ...
  const roles = (user?.roles?.length ? user.roles : ['LECTURER'])
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="min-w-0">
          <h1 className="text-[19px] font-medium text-[#1b1e26] tracking-tight leading-tight truncate">
            {user?.name || user?.username || 'Lecturer'}
          </h1>
          <p className="text-[12px] text-gray-500 mt-0.5 flex items-center gap-1.5 truncate">
            ...
            {user?.entityName || 'Lecturer'}
          </p>
        </div>
      </div>

      <div className="bg-white rounded-3xl border border-[#0B0E17]/[0.06] shadow-sm overflow-hidden">
        <div className="bg-[#f7f8fa] border-b border-[#0B0E17]/[0.05] p-2 flex items-center gap-1.5">
          {TABS.map((t) => {
        ...
        {activeTab === 'overview' && (
          <div className="p-6 sm:p-8 space-y-5 animate-in fade-in slide-in-from-bottom-1 duration-300" key="overview">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-0.5">Your account identity inside this institution.</p>
            </div>
        ...
        {activeTab === 'password' && (
          <div className="p-6 sm:p-8 animate-in fade-in slide-in-from-bottom-1 duration-300" key="password">
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-400 mt-0.5">Update your credentials to keep the account secure.</p>
            </div>
