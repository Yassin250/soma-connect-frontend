      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <input value={form.questionText} onChange={(e) => setForm((p) => ({ ...p, questionText: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#0B0E17]/30" placeholder="Enter the question…" />
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-1">Type</label>
            <select value={form.questionType} onChange={(e) => setForm((p) => ({ ...p, questionType: e.target.value, options: '', correctAnswer: '' }))} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#0B0E17]/30">
          ...
          <div>
            ...
            <input type="number" min="1" value={form.points} onChange={(e) => setForm((p) => ({ ...p, points: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#0B0E17]/30" />
          </div>
          {form.questionType === 'MULTIPLE_CHOICE' && (
            <div className="sm:col-span-2">
              ...
              <input value={form.options} onChange={(e) => setForm((p) => ({ ...p, options: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#0B0E17]/30 font-mono" placeholder='["Option A","Option B","Option C","Option D"]' />
            </div>
          )}
          <div className="sm:col-span-2">
            <label className="text-xs font-semibold text-gray-500 block mb-1">Correct answer</label>
            <input value={form.correctAnswer} onChange={(e) => setForm((p) => ({ ...p, correctAnswer: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#0B0E17]/30" placeholder={form.questionType === 'TRUE_FALSE' ? 'True or False' : 'Exactly as it appears in the options'} />
          </div>
        </div>
        <div className="mt-4 flex gap-2">
          <button onClick={handleSave} disabled={!form.questionText || !form.correctAnswer} className="px-5 py-2 rounded-xl bg-[#0B0E17] text-white text-sm font-semibold hover:bg-black disabled:opacity-50 transition-colors">{editing ? 'Update' : 'Add'} question</button>
