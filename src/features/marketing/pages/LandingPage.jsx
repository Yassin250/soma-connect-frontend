...
export const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-[#32292F] antialiased overflow-x-hidden scroll-smooth">
      {/* ───────────────── NAV ───────────────── */}
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#0A0A0A]/95 backdrop-blur-md shadow-lg py-3' : 'bg-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between">
          <Link to="/"><span style={{ '--clr-accent': '#3D7FFF' }}><BrandLockup /></span></Link>

          <nav className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map((l) => (
              <a key={l.href} href={l.href} className="text-sm font-medium text-white/70 hover:text-[#3D7FFF] transition-colors">{l.label}</a>
            ))}
          </nav>

          <div className="hidden lg:flex items-center gap-3">
            {isAuthenticated ? (
              <Link to={dashHref} className="px-5 py-2.5 rounded-xl bg-[#3D7FFF] text-white text-sm font-bold hover:bg-[#5C96FF] transition-colors">
                Go to Dashboard
              </Link>
            ) : (
              <>
                <Link to="/login" className="text-sm font-semibold text-white hover:text-[#3D7FFF] transition-colors">Sign In</Link>
                <Link to="/register" className="px-5 py-2.5 rounded-xl bg-[#3D7FFF] text-white text-sm font-bold hover:bg-[#5C96FF] transition-colors shadow-sm">Get Started</Link>
              </>
            )}
          </div>
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="lg:hidden mt-3 mx-4 rounded-2xl bg-[#0A0A0A] border border-white/10 p-4 space-y-1 shadow-xl">
            {NAV_LINKS.map((l) => (
            ...
            <div className="pt-2 flex gap-2">
              {isAuthenticated ? (
                <Link to={dashHref} className="flex-1 text-center px-4 py-2.5 rounded-xl bg-[#3D7FFF] text-white text-sm font-bold">Dashboard</Link>
              ) : (
                <>
                  <Link to="/login" className="flex-1 text-center px-4 py-2.5 rounded-xl border border-white/20 text-white text-sm font-semibold">Sign In</Link>
                   <Link to="/register" className="flex-1 text-center px-4 py-2.5 rounded-xl bg-[#3D7FFF] text-white text-sm font-bold">Get Started</Link>
                </>
              )}
            </div>
          </div>
      ...
      {/* ───────────────── HERO ───────────────── */}
      <section id="home" className="relative bg-gradient-to-br from-[#0A0A0A] via-[#0d0d12] to-[#08080a] text-white pt-36 pb-24 sm:pt-44 sm:pb-32 overflow-hidden">
        {/* ambient */}
        <div className="absolute -top-24 -left-24 w-[28rem] h-[28rem] rounded-full bg-[#3D7FFF]/20 blur-[120px] pointer-events-none" />
        <div className="absolute top-1/3 -right-20 w-96 h-96 rounded-full bg-[#1a1a1a]/40 blur-[110px] pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8 grid lg:grid-cols-2 gap-14 items-center">
          <div>
            ...
            >
              Learn with Integrity.<br />Lead with <span className="text-[#3D7FFF]">Opportunity.</span>
            </motion.h1>
            ...
            >
              <Link to="/register" className="px-7 py-3.5 rounded-2xl bg-[#3D7FFF] text-white text-sm font-bold hover:bg-[#5C96FF] transition-all active:scale-[0.98] shadow-lg shadow-[#3D7FFF]/20">
          ...
          >
            ...
            <div className="absolute -inset-10 rounded-full bg-[#3D7FFF]/25 blur-[90px] pointer-events-none" />
      ...
      <section id="features" className="bg-[#f7f8fa] py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <Reveal className="max-w-2xl mx-auto text-center mb-16">
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#2A6BE0] bg-[#3D7FFF]/15 px-3 py-1.5 rounded-full">Why Soma Connect</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 0.08}>
                <div className="h-full bg-white rounded-2xl border border-[#0A0A0A]/[0.09] shadow-[0_2px_10px_rgba(10,10,10,0.05)] p-6 hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
                  <span className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#3D7FFF] to-[#2A6BE0] text-white shadow-md flex items-center justify-center">
      ...
      <section id="programs" className="relative bg-[#f7f8fa] py-24 overflow-hidden">
        <div className="absolute -top-16 right-1/4 w-80 h-80 rounded-full bg-[#3D7FFF]/25 blur-[120px] pointer-events-none" />
        <div className="absolute bottom-0 -left-20 w-72 h-72 rounded-full bg-[#1a1a1a]/[0.06] blur-[100px] pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
          {/* Header + category filter */}
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-12">
            <Reveal className="max-w-xl">
              <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#32292F]/50 bg-[#3D7FFF]/40 px-3 py-1.5 rounded-full">Popular programs</span>
              <h2 className="mt-5 text-4xl sm:text-5xl font-semibold tracking-tight leading-[1.1]">
                ...
                <span className="relative inline-block">
                  ...
                  <svg className="absolute left-0 -bottom-1 w-full" height="10" viewBox="0 0 200 10" preserveAspectRatio="none" fill="none">
                    <path d="M2 7c40-5 80-5 120-3s60 3 76 1" stroke="#3D7FFF" strokeWidth="5" strokeLinecap="round" />
                  </svg>
                </span>
              </h2>
            ...
            <Reveal delay={0.1}>
              <div className="flex flex-wrap gap-2">
                {categoryPills.map((c) => (
                  ...
                  >
                    {c.label}
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${programCat === c.label ? 'bg-white/15' : 'bg-[#0A0A0A]/[0.06]'}`}>
                      {c.count}
                    </span>
                  </button>
                ))}
                ...
                >
                    ...
                    <div className="p-5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {p.certificate && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-[#3D7FFF]/30 px-2 py-0.5 rounded-full">
                      ...
                      <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between gap-2">
                        {p.instructor ? (
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="w-7 h-7 rounded-full bg-[#0A0A0A] text-white text-[9px] font-bold flex items-center justify-center shrink-0">{nameInitials(p.instructor)}</span>
                      ...
                      <button
                        ...
                        className="mt-4 w-full py-2.5 rounded-xl bg-[#0A0A0A] text-white text-sm font-bold hover:bg-black transition-colors active:scale-[0.98] inline-flex items-center justify-center gap-2 group-hover:bg-[#3D7FFF] group-hover:text-white"
                      >
          ...
          <Reveal className="mt-12 text-center">
            <Link to="/register" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-2xl bg-[#0A0A0A] text-white text-sm font-bold hover:bg-black transition-colors active:scale-[0.98]">
      ...
      {/* ───────────────── HOW IT WORKS ───────────────── */}
      <section id="how" className="relative bg-gradient-to-br from-[#0A0A0A] via-[#0d0d12] to-[#08080a] text-white py-24 overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-[#3D7FFF]/10 blur-[120px] pointer-events-none" />
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
          <Reveal className="max-w-2xl mb-16">
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#3D7FFF] bg-[#3D7FFF]/10 border border-[#3D7FFF]/20 px-3 py-1.5 rounded-full">How it works</span>
            ...
            {STEPS.map((s, i) => (
              <Reveal key={s.n} delay={i * 0.08}>
                <div className="relative rounded-2xl bg-white/[0.03] border border-white/10 p-6 h-full">
                  <span className="text-4xl font-bold text-[#3D7FFF]/30">{s.n}</span>
      ...
      <section className="bg-[#f7f8fa] py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <Reveal>
            <div className="relative rounded-[2rem] bg-gradient-to-br from-[#0A0A0A] to-[#08080a] text-white p-10 sm:p-14 overflow-hidden">
              <div className="absolute -bottom-16 -right-10 w-72 h-72 rounded-full bg-[#3D7FFF]/15 blur-[90px] pointer-events-none" />
              <div className="relative grid lg:grid-cols-2 gap-10 items-center">
                <div>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#3D7FFF]">For institutions</span>
                  ...
                  <div className="mt-8 flex flex-wrap gap-3">
                    <Link to="/register" className="px-6 py-3 rounded-2xl bg-[#3D7FFF] text-white text-sm font-bold hover:bg-[#5C96FF] transition-colors">Partner with us</Link>
                ...
                <div className="space-y-3">
                  {['Unified school dashboard', 'Bulk student & staff onboarding', 'Attendance & performance analytics', 'AI-plagiarism & originality checks'].map((t) => (
                    <div key={t} className="flex items-center gap-3 rounded-2xl bg-white/[0.04] border border-white/5 px-4 py-3.5">
                        <span className="w-7 h-7 rounded-lg bg-[#3D7FFF] text-white flex items-center justify-center shrink-0">
      ...
      <section className="bg-white py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <Reveal className="max-w-2xl mx-auto text-center mb-14">
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#32292F]/50 bg-[#3D7FFF]/40 px-3 py-1.5 rounded-full">Loved by learners</span>
            ...
            {TESTIMONIALS.map((t, i) => (
              <Reveal key={t.name} delay={i * 0.08}>
                <div className="h-full bg-[#f7f8fa] rounded-2xl border border-gray-100 p-7 flex flex-col">
                  <svg className="w-8 h-8 text-[#3D7FFF]" viewBox="0 0 24 24" fill="currentColor"><path d="M9.5 8c-2.5 0-4.5 2-4.5 4.5S7 17 9.5 17c.3 0 .6 0 .9-.1-.6 1.3-2 2.3-3.9 2.6l.4 1.5c3.6-.7 6.1-3.4 6.1-7.2V12.5C13 9.5 11.5 8 9.5 8zm9 0c-2.5 0-4.5 2-4.5 4.5S16 17 18.5 17c.3 0 .6 0 .9-.1-.6 1.3-2 2.3-3.9 2.6l.4 1.5c3.6-.7 6.1-3.4 6.1-7.2V12.5C22 9.5 20.5 8 18.5 8z" /></svg>
                  <p className="mt-4 text-[15px] text-gray-700 leading-relaxed flex-1">“{t.quote}”</p>
                  <div className="mt-6 flex items-center gap-3">
                    <span className="w-10 h-10 rounded-full bg-[#0A0A0A] text-white text-xs font-bold flex items-center justify-center">{t.name.split(' ').map((w) => w[0]).join('')}</span>
                    <div>
      ...
      <section id="pricing" className="bg-[#f7f8fa] py-24">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          <Reveal className="max-w-2xl mx-auto text-center mb-14">
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#32292F]/50 bg-[#3D7FFF]/40 px-3 py-1.5 rounded-full">Pricing</span>
          ...
          <div className="grid md:grid-cols-3 gap-6 items-stretch">
            {PLANS.map((p, i) => (
              <Reveal key={p.name} delay={i * 0.08}>
                <div className={`h-full rounded-2xl p-8 flex flex-col ${p.featured ? 'bg-[#0A0A0A] text-white shadow-2xl scale-[1.03] relative' : 'bg-white border border-gray-100'}`}>
                  {p.featured && <span className="absolute top-5 right-5 text-[10px] font-bold uppercase tracking-wide text-white bg-[#3D7FFF] px-2.5 py-1 rounded-full">Most popular</span>}
                  <p className={`text-sm font-semibold ${p.featured ? 'text-[#3D7FFF]' : 'text-gray-500'}`}>{p.name}</p>
                  ...
                  <ul className="mt-6 space-y-3 flex-1">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-start gap-2.5 text-sm">
                        <svg className={`w-5 h-5 shrink-0 ${p.featured ? 'text-[#3D7FFF]' : 'text-emerald-500'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        <span className={p.featured ? 'text-white/80' : 'text-gray-600'}>{f}</span>
                      </li>
                    ))}
                  </ul>
                  {p.name === 'Institution' ? (
                    <a href="https://wa.me/250794219712?text=Hello%20Soma%20Connect%2C%20I%27m%20interested%20in%20the%20Institution%20plan" target="_blank" rel="noopener noreferrer" className="block mt-8 text-center px-6 py-3 rounded-2xl text-sm font-bold transition-colors bg-[#0A0A0A] text-white hover:bg-black">
                      {p.cta}
                    </a>
                  ) : (
                    <Link to="/register" className={`block mt-8 text-center px-6 py-3 rounded-2xl text-sm font-bold transition-colors ${p.featured ? 'bg-[#3D7FFF] text-white hover:bg-[#5C96FF]' : 'bg-[#0A0A0A] text-white hover:bg-black'}`}>
                      {p.cta}
                    </Link>
                  )}
                </div>
      ...
      <section className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <Reveal>
            <div className="relative rounded-[2rem] bg-[#3D7FFF] p-12 sm:p-16 text-center overflow-hidden">
              <div className="absolute -top-10 -left-8 w-40 h-40 rounded-full bg-white/25 pointer-events-none" />
              <div className="absolute -bottom-12 -right-6 w-56 h-56 rounded-full bg-[#0A0A0A]/5 pointer-events-none" />
              <div className="relative">
                ...
                <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
                  <Link to="/register" className="px-8 py-4 rounded-2xl bg-[#0A0A0A] text-white text-sm font-bold hover:bg-black transition-colors active:scale-[0.98]">Create your free account</Link>
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mt-5 flex gap-3">
                {['M22 12a10 10 0 10-11.5 9.9v-7H8v-2.9h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5h-1.2c-1.2 0-1.6.8-1.6 1.6v1.9h2.7l-.4 2.9h-2.3v7A10 10 0 0022 12z', 'M23 4.9c-.8.4-1.7.6-2.6.8a4.5 4.5 0 002-2.5c-.9.5-1.9.9-2.9 1.1a4.5 4.5 0 00-7.7 4.1A12.8 12.8 0 013 3.6a4.5 4.5 0 001.4 6 4.5 4.5 0 01-2-.6v.1a4.5 4.5 0 003.6 4.4 4.5 4.5 0 01-2 .1 4.5 4.5 0 004.2 3.1A9 9 0 012 19.5a12.7 12.7 0 006.9 2c8.3 0 12.8-6.9 12.8-12.8v-.6c.9-.6 1.6-1.4 2.3-2.2z', 'M19 3a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h14zM8.5 17.5v-7H6v7h2.5zM7.2 9.3a1.4 1.4 0 100-2.8 1.4 1.4 0 000 2.8zM18 17.5v-4c0-2.1-1.1-3.1-2.6-3.1-1.2 0-1.7.7-2 1.1v-1H11v7h2.5v-3.9c0-.9.6-1.3 1.2-1.3.6 0 1.1.4 1.1 1.3v3.9H18z'].map((d, i) => (
                  <a key={i} href="#" className="w-9 h-9 rounded-lg bg-white/5 hover:bg-[#3D7FFF] hover:text-white text-white/60 flex items-center justify-center transition-colors">
            ...
            {FOOTER.map((col) => (
              <div key={col.title}>
                ...
                <ul className="space-y-2.5">
                  {col.links.map((l) => (
                    <li key={l}><a href="#" className="text-sm text-white/50 hover:text-[#3D7FFF] transition-colors">{l}</a></li>
                  ))}
                </ul>
              </div>
            ))}
