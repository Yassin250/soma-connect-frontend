...
export const SchoolAccountPage = () => {
  const { user } = useAuth();
  ...
  // "ENTITY_ADMIN" reads as "Entity Admin" — same tone as the other values.
  const roles = (user?.roles?.length ? user.roles : ['ENTITY_ADMIN'])
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[12px] text-gray-500 mt-0.5 flex items-center gap-1.5 truncate">
            ...
            {user?.entityName || 'Your Institution'}
          </p>
        </div>
      </div>
