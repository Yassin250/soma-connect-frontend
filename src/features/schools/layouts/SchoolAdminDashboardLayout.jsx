...
export const SchoolAdminDashboardLayout = () => {
  ...
  // ── Lecturer branch — lightweight full-page portal ──────────────────────────
  if (isLecturer) {
    return (
      <div className="w-screen h-screen bg-[#f3f4f6] text-[#1b1e26] flex flex-col overflow-hidden antialiased">
        <nav className="border-b border-gray-100 bg-white shrink-0 z-40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-[72px]">
              <div className="flex items-center gap-3">
                <BrandLockup dark size={34} />
                <span className="px-2.5 py-1 text-[10px] font-bold bg-[#d0f24a]/25 text-[#1b1e26] rounded-lg uppercase tracking-wider">Faculty Hub</span>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-semibold text-[#1b1e26] leading-tight">{user.name}</p>
                  <p className="text-[11px] text-gray-400">Lecturer</p>
                </div>
                <button
                  onClick={handleLogout}
                  className="text-xs px-4 py-2.5 bg-[#1b1e26] hover:bg-black text-white rounded-xl font-semibold transition-colors"
                >
                  Sign Out
                </button>
              </div>
